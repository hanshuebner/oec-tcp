...
class Device:
    ...
    def execute(self, commands):
        ...
        return self.interface.execute(commands)
...
def format_device(interface):
    """Format device identifier."""
    return f'{interface.identifier}'

def get_ids(interface, extended_id_retry_attempts=3):
    terminal_id = None
    ...
    if terminal_id is not None and terminal_id.type != TerminalType.DFT:
        ...
        try:
            extended_id = interface.execute(commands)[-1]
